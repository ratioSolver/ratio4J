/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class it_cnr_istc_pst_oratio_TimelinesExecutor */

#ifndef _Included_it_cnr_istc_pst_oratio_TimelinesExecutor
#define _Included_it_cnr_istc_pst_oratio_TimelinesExecutor
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    new_instance
 * Signature: (JJ)J
 */
JNIEXPORT jlong JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_new_1instance
  (JNIEnv *, jobject, jlong, jlong);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    dispose
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_dispose
  (JNIEnv *, jobject);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    adapt
 * Signature: (Ljava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_adapt__Ljava_lang_String_2
  (JNIEnv *, jobject, jstring);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    adapt
 * Signature: ([Ljava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_adapt___3Ljava_lang_String_2
  (JNIEnv *, jobject, jobjectArray);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    is_executing
 * Signature: ()Z
 */
JNIEXPORT jboolean JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_is_1executing
  (JNIEnv *, jobject);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    start_execution
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_start_1execution
  (JNIEnv *, jobject);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    pause_execution
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_pause_1execution
  (JNIEnv *, jobject);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    is_finished
 * Signature: ()Z
 */
JNIEXPORT jboolean JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_is_1finished
  (JNIEnv *, jobject);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    tick
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_tick
  (JNIEnv *, jobject);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    dont_start_yet
 * Signature: ([J[J[J)V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_dont_1start_1yet
  (JNIEnv *, jobject, jlongArray, jlongArray, jlongArray);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    dont_end_yet
 * Signature: ([J[J[J)V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_dont_1end_1yet
  (JNIEnv *, jobject, jlongArray, jlongArray, jlongArray);

/*
 * Class:     it_cnr_istc_pst_oratio_TimelinesExecutor
 * Method:    failure
 * Signature: ([J)V
 */
JNIEXPORT void JNICALL Java_it_cnr_istc_pst_oratio_TimelinesExecutor_failure
  (JNIEnv *, jobject, jlongArray);

#ifdef __cplusplus
}
#endif
#endif
