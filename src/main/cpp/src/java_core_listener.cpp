...
namespace ratio::java
{
    ...
    java_core_listener::java_core_listener(ratio::core::core &cr, JNIEnv *env, jobject obj) : scoped_env(env), core_listener(cr), slv_obj(env->NewGlobalRef(obj)), solver_cls(reinterpret_cast<jclass>(env->NewGlobalRef(env->GetObjectClass(obj)))),
    ...
    void java_core_listener::state_changed()
    {
        ...
        for (const auto &[itm, j_itm] : std::unordered_map<const ratio::core::item *, jobject>(all_items))
        {
            ...
            if (const auto a = dynamic_cast<const ratio::core::atom *>(itm))
            {
                ...
                env->CallVoidMethod(j_itm, atm_set_state_mthd_id, slv.get_sat_core()->value(get_sigma(slv, *a)));
            }
        }
    ...
    void java_core_listener::new_type(JNIEnv *env, const ratio::core::type &t)
    {
        jstring t_name = env->NewStringUTF(t.get_name().c_str());
        jobject c_type;

        if (is_core(t.get_scope()))
        {
        ...
        {
            jobject c_scope = all_types.at(static_cast<ratio::core::type *>(&t.get_scope()));
    ...
    void java_core_listener::revise_type(JNIEnv *env, const ratio::core::type &t)
    {
    ...
    void java_core_listener::new_predicate(JNIEnv *env, const ratio::core::predicate &p)
    {
        ...
        {
            jobject c_scope = all_types.at(static_cast<ratio::core::type *>(&p.get_scope()));
    ...
    void java_core_listener::revise_predicate(JNIEnv *env, const ratio::core::predicate &p)
    {
    ...
    void java_core_listener::new_item(JNIEnv *env, const ratio::core::item &itm)
    {
        jobject c_type = all_types.at(&itm.get_type());

        jobject c_item = env->NewGlobalRef(env->NewObject(item_cls, item_ctr_id, slv_obj, c_type, get_id(itm)));
    ...
    void java_core_listener::new_atom(JNIEnv *env, const ratio::core::atom &atm)
    {
        jobject c_pred = all_types.at(&atm.get_type());

        auto &slv = static_cast<ratio::solver::solver &>(cr);
        jobject c_atom = env->NewGlobalRef(env->NewObject(atm_cls, atm_ctr_id, slv_obj, c_pred, get_id(atm), get_sigma(slv, atm), slv.get_sat_core()->value(get_sigma(slv, atm))));
        env->CallVoidMethod(c_pred, t_new_instnc, c_atom);
        all_items.emplace(&atm, c_atom);
    }

    jobject java_core_listener::new_field(JNIEnv *env, const std::string &name, const ratio::core::type &tp)
    {
    ...
    jobjectArray java_core_listener::new_fields_array(JNIEnv *env, const std::vector<const ratio::core::field *> &args)
    {
    ...
    jobject java_core_listener::set(JNIEnv *env, jobject c_obj, jmethodID mthd_id, const std::string &name, const ratio::core::item &itm)
    {
        auto &slv = static_cast<ratio::solver::solver &>(cr);
        ...
        { // the expression represents a primitive bool type..
            ...
            { // we create a new boolean..
                jstring lit_s = env->NewStringUTF(((sign(bi->get_value()) ? "b" : "!b") + std::to_string(variable(bi->get_value()))).c_str());
                new_obj = env->NewGlobalRef(env->NewObject(bool_item_cls, bool_item_ctr_id, slv_obj, get_id(*bi), lit_s, c_val));
        ...
        { // the expression represents a primitive arithmetic type..
            ...
            { // we create a new arith..
                jobject c_type = all_types.at(&itm.get_type());
                jstring lit_s = env->NewStringUTF(to_string(ai->get_value()).c_str());
                ...
                new_obj = env->NewGlobalRef(env->NewObject(arith_item_cls, arith_item_ctr_id, slv_obj, c_type, get_id(*ai), lit_s, c_lb, c_ub, c_val));
                env->CallVoidMethod(c_obj, mthd_id, i_name, new_obj);
        ...
        { // the expression represents an enum type..
            ...
            for (const auto &v : vals)
                env->SetObjectArrayElement(vals_array, static_cast<jsize>(i++), all_items.at(static_cast<const ratio::core::item *>(v)));

            if (i_it == all_items.cend())
            { // we create a new enum..
                jobject c_type = all_types.at(&itm.get_type());
                jstring lit_s = env->NewStringUTF(("e" + std::to_string(ei->get_var())).c_str());
                new_obj = env->NewGlobalRef(env->NewObject(enum_item_cls, enum_item_ctr_id, slv_obj, c_type, get_id(*ei), lit_s, vals_array));
                env->CallVoidMethod(c_obj, mthd_id, i_name, new_obj);
        ...
        { // the expression represents a primitive string type..
            ...
            if (i_it == all_items.cend())
            {
                new_obj = env->NewGlobalRef(env->NewObject(string_item_cls, string_item_ctr_id, slv_obj, get_id(*si), c_val));
                env->CallVoidMethod(c_obj, mthd_id, i_name, new_obj);
