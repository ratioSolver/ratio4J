...
{
    java_solver_listener::java_solver_listener(ratio::solver::solver &s, JNIEnv *env, jobject obj) : scoped_env(env), solver_listener(s), slv_obj(env->NewGlobalRef(obj)), solver_cls(reinterpret_cast<jclass>(env->NewGlobalRef(env->GetObjectClass(obj)))),
    ...
    void java_solver_listener::flaw_created(const ratio::solver::flaw &f)
    {
        const auto &env = get_env();
        // the flaw's id..
        ...
        // the flaw's label..
        jstring label = env->NewStringUTF(f.get_data().dump().c_str());
    ...
    void java_solver_listener::flaw_state_changed(const ratio::solver::flaw &f)
    {
        // the flaw's id..
        jlong id = reinterpret_cast<jlong>(&f);
    ...
    void java_solver_listener::flaw_cost_changed(const ratio::solver::flaw &f)
    {
        // the flaw's id..
        jlong id = reinterpret_cast<jlong>(&f);
    ...
    void java_solver_listener::flaw_position_changed(const ratio::solver::flaw &f)
    {
        // the flaw's id..
        jlong id = reinterpret_cast<jlong>(&f);
    ...
    void java_solver_listener::current_flaw(const ratio::solver::flaw &f)
    {
        // the flaw's id..
        jlong id = reinterpret_cast<jlong>(&f);
    ...
    void java_solver_listener::resolver_created(const ratio::solver::resolver &r)
    {
        const auto &env = get_env();
        // the resolver's id..
        ...
        jstring label = env->NewStringUTF(r.get_data().dump().c_str());
    ...
    void java_solver_listener::resolver_state_changed(const ratio::solver::resolver &r)
    {
        // the resolver's id..
        jlong id = reinterpret_cast<jlong>(&r);
    ...
    void java_solver_listener::current_resolver(const ratio::solver::resolver &r)
    {
        // the resolver's id..
        jlong id = reinterpret_cast<jlong>(&r);
    ...
    void java_solver_listener::causal_link_added(const ratio::solver::flaw &f, const ratio::solver::resolver &r)
    {
        // the flaw's id..
